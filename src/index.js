...
export const render = data => {
  ...
  svg.selectAll('.delete-icon').on('click', function ({ id }) {
    links = removeLinksRelations(links, id)
    nodes = nodes.filter(node => id !== node.id)
    render({ links, nodes })

    // todo dispatch custom event "remove"
    d3.select(this).on('remove', function (d, i) {
      var evt = d3.event
      //  console.log(evt);
    })

    d3.select('p').dispatch('remove', {
      detail: { myCustomObject: 'bla bla bla' }
    })
  })
  ...
  simulation.nodes(nodes).on('tick', () => {
    ticked(link, node, edgepaths, edgelabels)
  })
